...
function c101103056.initial_effect(c)
	...
	e2:SetCountLimit(1,101103057+100)
...
function c101103056.atkfilter(c)
	return c:IsRace(RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINDBEAST) and c:IsType(TYPE_LINK) and c:IsLinkState()
end
function c101103056.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c101103056.atkfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	local g=Duel.SelectTarget(tp,c101103056.atkfilter,tp,LOCATION_MZONE,0,1,99,e:GetHandler())
end
...
function c101103056.replaceft(c,tp)
	return c:IsFaceup() and c:IsRace(RACE_BEAST+RACE_BEASTWARRIOR+RACE_WINDBEAST) and c:IsType(TYPE_LINK) and c:IsLinkState()
