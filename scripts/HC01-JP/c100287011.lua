...
--
--Script by Trishula9
function c100287011.initial_effect(c)
	--activate
	...
	e2:SetDescription(aux.Stringid(100287011,0))
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c100287011.descon)
	e2:SetTarget(c100287011.destg)
	e2:SetOperation(c100287011.desop)
	c:RegisterEffect(e2)
	--damage
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(100287011,1))
	...
	e3:SetCountLimit(1)
	e3:SetCondition(c100287011.damcon)
	e3:SetTarget(c100287011.damtg)
	e3:SetOperation(c100287011.damop)
	c:RegisterEffect(e3)
	--atk up
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(100287011,2))
	...
	e4:SetCountLimit(1)
	e4:SetCondition(c100287011.atkcon)
	e4:SetTarget(c100287011.atktg)
	e4:SetOperation(c100287011.atkop)
	c:RegisterEffect(e4)
end
function c100287011.filter1(c)
...
function c100287011.desop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		Duel.Destroy(tc,REASON_EFFECT)
	end
end
function c100287011.filter2(c)
...
function c100287011.damop(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Damage(p,d,REASON_EFFECT)
end
function c100287011.filter3(c)
...
function c100287011.atkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
