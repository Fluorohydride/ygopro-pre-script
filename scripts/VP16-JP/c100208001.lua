...
function c100208001.atcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(Card.IsSetCard,tp,LOCATION_HAND+LOCATION_DECK,0,nil,0xc008)
	if g:GetCount()==1 then
		if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,g:GetFirst()) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DISCARD)
		local tg=Duel.SelectMatchingCard(tp,Card.IsDiscardable,tp,LOCATION_HAND,0,1,1,g:GetFirst())
...
function c100208001.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,Card.IsSetCard,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,0xc008)
	local tc=g:GetFirst()
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_GRAVE) then
		local tg=Duel.GetMatchingGroup(c100208001.atfilter,tp,LOCATION_MZONE,0,nil,0xc008)
		if tg:GetCount()<=0 then return end
		local sg=Duel.GetMatchingGroupCount(Card.IsSetCard,tp,LOCATION_GRAVE,0,nil,0xc008)
		tc=tg:GetFirst()
		while tc do
