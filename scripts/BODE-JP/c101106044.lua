...
function c101106044.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(101106044,0))
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1,101106044)
	e2:SetCondition(c101106044.setcon)
	e2:SetTarget(c101106044.settg)
	e2:SetOperation(c101106044.setop)
	c:RegisterEffect(e2)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e3:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e3:SetCode(EVENT_SPSUMMON_SUCCESS)
	e3:SetCondition(c101106044.matcon)
	e3:SetOperation(c101106044.matop)
	c:RegisterEffect(e3)
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetCode(EFFECT_MATERIAL_CHECK)
	e4:SetLabelObject(e3)
	e4:SetValue(c101106044.valcheck)
	c:RegisterEffect(e4)
	--destroy
