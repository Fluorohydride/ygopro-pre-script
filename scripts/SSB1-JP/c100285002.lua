...
function c100285002.tgfilter(c,e,tp)
	return c:IsAbleToGrave()
		and Duel.IsExistingMatchingCard(c100285002.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function c100285002.spfilter(c,e,tp,mc)
...
function c100285002.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100285002.tgfilter,tp,LOCATION_ONFIELD,0,1,e:GetHandler(),e,tp) end
...
function c100285002.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,c100285002.tgfilter,tp,LOCATION_ONFIELD,0,1,1,c,e,tp):GetFirst()
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,c100285002.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc):GetFirst()
		if sc then
