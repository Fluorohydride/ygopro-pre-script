...
function c100331002.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(100331002,3))
	...
	e3:SetDescription(aux.Stringid(100331002,5))
...
function c100331002.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.SelectYesNo(tp,aux.Stringid(100331002,4)) then
		Duel.BreakEffect()
...
function c100331002.hncost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsAbleToRemoveAsCost()
		and Duel.IsExistingMatchingCard(c100331002.hncfilter,tp,loc,0,1,nil,0x10f2)
		and Duel.IsExistingMatchingCard(c100331002.hncfilter,tp,loc,0,1,nil,0x2073)
		and Duel.IsExistingMatchingCard(c100331002.hncfilter,tp,loc,0,1,nil,0x2017)
		and Duel.IsExistingMatchingCard(c100331002.hncfilter,tp,loc,0,1,nil,0x1046)
	end
	local g=Group.FromCards(c)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g1=Duel.SelectMatchingCard(tp,c100331002.hncfilter,tp,loc,0,1,1,nil,0x10f2)
	g:Merge(g1)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=Duel.SelectMatchingCard(tp,c100331002.hncfilter,tp,loc,0,1,1,nil,0x2073)
	g:Merge(g2)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g3=Duel.SelectMatchingCard(tp,c100331002.hncfilter,tp,loc,0,1,1,nil,0x2017)
	g:Merge(g3)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g4=Duel.SelectMatchingCard(tp,c100331002.hncfilter,tp,loc,0,1,1,nil,0x1046)
	g:Merge(g4)
