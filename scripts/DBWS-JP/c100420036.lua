...
function c100420036.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100420036.filter,tp,LOCATION_DECK,0,2,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c100420036.thfilter(c,tc)
	local code=0
...
function c100420036.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 and tc:IsRace(RACE_BEASTWARRIOR+RACE_WARRIOR)
		and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c100420036.thfilter),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,attr)
		and Duel.SelectYesNo(tp,aux.Stringid(100420036,1)) then
		Duel.BreakEffect()
		g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c100420036.thfilter),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,attr)
		if #g>0 then
