...
function c100412003.thfilter(c)
	return c:IsAbleToHand() and c:IsCode(100412106)
end
...
function c100412003.cfilter(c,ft,tp)
	return c:IsRace(RACE_MACHINE) and c:IsAttribute(ATTRIBUTE_EARTH)
		and (ft>0 or (c:IsControler(tp) and (not c:IsLocation(LOCATION_EXTRA))))
end
...
function c100412003.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>-1 and Duel.CheckReleaseGroup(tp,c100412003.cfilter,1,nil,ft,tp) end
	local g=Duel.SelectReleaseGroup(tp,c100412003.cfilter,1,1,nil,ft,tp)
