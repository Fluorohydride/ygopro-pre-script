...
function c100412011.immunefilter(e,te)
	return not (te:GetHandler()):IsType(TYPE_XYZ)
end
function c100412011.indes(e,c)
	return not c:IsType(TYPE_XYZ)
end
function c100412011.matfilter(c)
	return c:IsCanBeXyzMaterial() and (not c:IsType(TYPE_TOKEN)) and c:IsAbleToChangeControler()
end
function c100412011.spfilter(c,e,tp)
	return c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(c100412011.matfilter,tp,0,LOCATION_ONFIELD,1,nil)
end
function c100412011.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c100412011.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c100412011.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,g:GetCount(),0,0)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(100412011,0))
...
function c100412011.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetCondition(c100412011.damcon)
	e1:SetOperation(c100412011.dop)
	e1:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e1,tp)
end
function c100412011.damcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function c100412011.dop(e,tp,eg,ep,ev,re,r,rp)
