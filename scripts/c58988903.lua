...
function c58988903.initial_effect(c)
	...
	e2:SetCondition(c58988903.spcon)
...
function c58988903.filter1(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xba) and c:IsType(TYPE_XYZ)
		and Duel.IsExistingMatchingCard(c58988903.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c)
end
function c58988903.filter2(c,e,tp,mc,rk)
	return c:IsSetCard(0xba) and c:GetRank()==mc:GetRank()+2 and mc:IsCanBeXyzMaterial(c)
...
function c58988903.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c58988903.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc)
	local sc=g:GetFirst()
	if sc then
...
function c58988903.spcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnCount()~=e:GetHandler():GetTurnID()
end
function c58988903.spcfilter(c,e,tp)
	if c:IsSetCard(0xba) and c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.GetMatchingGroupCount(c58988903.spfilter,tp,LOCATION_GRAVE,0,nil,e,tp)==1 then return false end
...
function c58988903.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() and Duel.IsExistingMatchingCard(c58988903.spcfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	local tg=Duel.SelectMatchingCard(tp,c58988903.spcfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c58988903.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_GRAVE) and c58988903.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c58988903.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c58988903.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
