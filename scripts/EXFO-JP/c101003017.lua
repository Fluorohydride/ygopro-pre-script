...
function c101003017.initial_effect(c)
	...
	e3:SetOperation(c101003017.activate)
	c:RegisterEffect(e3)
end
...
function c101003017.hspval(e,c)
	local zone=0
	for i=0,4 do
		if Duel.GetMatchingGroupCount(c101003017.cfilter,c:GetControler(),LOCATION_ONFIELD,LOCATION_ONFIELD,nil,tp,i)>=2 then
			zone=zone+math.pow(2,i)
		end
	end
	return 0,zone
...
function c101003017.filter(c,hc)
  return c:IsType(TYPE_SPELL+TYPE_TRAP) and c:GetColumnGroup():IsContains(hc)
end
function c101003017.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  local c=e:GetHandler()
	if chkc then return c101003017.filter(chkc,c) and chkc:IsOnField() end
	if chk==0 then return Duel.IsExistingTarget(c101003017.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c101003017.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,c)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
end
function c101003017.activate(e,tp,eg,ep,ev,re,r,rp)
