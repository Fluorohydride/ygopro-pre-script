...
function c101003018.initial_effect(c)
	...
	e3:SetOperation(c101003018.activate)
	c:RegisterEffect(e3)
end
...
function c101003018.hspval(e,c)
	local zone=0
	for i=0,4 do
		if Duel.GetMatchingGroupCount(c101003018.cfilter,c:GetControler(),LOCATION_ONFIELD,LOCATION_ONFIELD,nil,tp,i)>=2 then
			zone=zone+math.pow(2,i)
		end
	end
	return 0,zone
...
function c101003018.filter(c,hc)
  return c:IsFaceup() and c:GetColumnGroup():IsContains(hc)
end
function c101003018.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  local c=e:GetHandler()
	if chkc then return c101003018.filter(chkc,c) and chkc:IsLocation(LOCATION_MZONE) and chkc~=c end
	if chk==0 then return Duel.IsExistingTarget(c101003018.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c101003018.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,c,c)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
end
function c101003018.activate(e,tp,eg,ep,ev,re,r,rp)
