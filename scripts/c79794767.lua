...
function c79794767.initial_effect(c)
	...
	e3:SetCondition(c79794767.rcon)
	e3:SetTarget(c79794767.rlimit)
	...
	e4:SetOperation(c79794767.flagop)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	e5:SetCode(EVENT_SPSUMMON_SUCCESS)
	e5:SetCondition(c79794767.flagcon)
	c:RegisterEffect(e5)
	...
	e6:SetCondition(c79794767.spcon)
	e6:SetTarget(c79794767.splimit)
	c:RegisterEffect(e6)
end
...
function c79794767.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local op=false
	if con then op=Duel.SelectYesNo(tp,aux.Stringid(79794767,0)) end
	if op then
...
function c79794767.rfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsSetCard(0x1d1)
end
function c79794767.rcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c79794767.rfilter,tp,LOCATION_ONFIELD,0,1,nil)
end
function c79794767.rlimit(e,c)
	return not c:IsSetCard(0x1d1)
end
function c79794767.flagcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_PENDULUM
end
function c79794767.flagop(e,tp,eg,ep,ev,re,r,rp)
...
function c79794767.splimit(e,c,sump,sumtype,sumpos,targetp)
