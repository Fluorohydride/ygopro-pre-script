function c100209281.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(100209281,0))
...
function c100209281.selfilter(c,e,tp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		return (c:IsSetCard(0xa7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)) or (c:IsSetCard(0x207a) and c:IsAbleToHand()) 
	else
	...
	if c:IsRelateToEffect(e) and Duel.Remove(c,0,REASON_EFFECT+REASON_TEMPORARY)~=0
		...
		if g:GetCount()>0 then
			...
			if Duel.MoveToField(tc,tp,tp,LOCATION_FZONE,POS_FACEUP,true) 
					...
					and Duel.SelectYesNo(tp,aux.Stringid(100209281,1)) 
			then
				Duel.BreakEffect()
