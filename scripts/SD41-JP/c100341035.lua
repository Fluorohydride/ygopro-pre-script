...
function c100341035.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100341035,0))
	...
	e1:SetTarget(c100341035.eqtg)
	e1:SetOperation(c100341035.eqop)
	c:RegisterEffect(e1)
	--destroy
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100341035,1))
...
function c100341035.eqop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c100341035.eqfilter2),tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil,ec,tp)
		local ec=g:GetFirst()
		...
		e1:SetValue(c100341035.eqlimit)
...
function c100341035.eqlimit(e,c)
	return c==e:GetLabelObject()
end
function c100341035.cfilter(c)
	return c:IsType(TYPE_EQUIP) and c:GetEquipTarget():IsRace(RACE_MACHINE) and c:IsAbleToGrave()
end
...
function c100341035.desop(e,tp,eg,ep,ev,re,r,rp)
