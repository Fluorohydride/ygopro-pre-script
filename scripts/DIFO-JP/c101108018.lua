...
function c101108018.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=tp
		local mg1=Duel.GetFustionMaterial(tp)
...
function c101108018.fusop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetFustionMaterial(tp):Filter(c101108018.fusfilter1,nil,e)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			local mat1=Duel.SelectFustionMaterial(tp,tc,mg1,nil,chkf)
			tc:SetMaterial(mat1)
...
function c101108018.thcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsPreviousLocation(LOCATION_DECK+LOCATION_EXTRA+LOCATION_HAND+LOCATION_ONFIELD+LOCATION_REMOVED)
		and c:IsLocation(LOCATION_GRAVE+LOCATION_EXTRA) and r==REASON_FUSION
end
function c101108018.thfilter(c)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:IsType(TYPE_PENDULUM) and not c:IsCode(101108018) and c:IsAbleToHand()
end
