...
function c101108074.filter(c,e,tp)
	return (c:IsSetCard(0x27b) and c:IsType(TYPE_MONSTER)) or c:IsCode(101108008)
end
...
function c101108074.adatkop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetValue(function(e) local c = e:GetHandler() 
		return (c:GetAttack()>c:GetDefense() and {0} or {1})[1] end)
	tc:RegisterEffect(e1)
end
function c101108074.exfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x27b) and c:IsType(TYPE_MONSTER) and c:GetSequence()>=5
end
