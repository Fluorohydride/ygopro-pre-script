...
function c101004084.mfilterf(c,tp,mg,dg,rc)
	if c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5 then
		...
		return mg:CheckWithSumEqual(Card.GetRitualLevel,rc:GetLevel(),0,99,rc)
			or dg:IsExists(c101004084.dlvfilter,1,nil,tp,mg,rc,rc:GetLevel()-c:GetRitualLevel(rc))
...
function c101004084.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		tc:RegisterFlagEffect(101004084,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,2)
