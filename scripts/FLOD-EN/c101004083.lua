...
function c101004083.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x106) and not c:IsCode(101004083) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c101004083.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c101004083.spfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c101004083.spfilter,tp,LOCATION_REMOVED,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c101004083.spfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp)
...
function c101004083.mtop(e,tp,eg,ep,ev,re,r,rp)
	...
	rc:RegisterFlagEffect(0,RESET_EVENT+RESETS_STANDARD,EFFECT_FLAG_CLIENT_HINT,1,0,aux.Stringid(101004083,1))
