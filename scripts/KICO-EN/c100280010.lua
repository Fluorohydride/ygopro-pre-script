...
function c100280010.initial_effect(c)
	...
	e2:SetCondition(c100280010.con)
	e2:SetTarget(c100280010.tg)
	e2:SetOperation(c100280010.op)
	c:RegisterEffect(e2)
end
function c100280010.cfilter(c)
	return c:IsFaceup() and (c:IsCode(91998119) or aux.IsMaterialListCode(c,91998119))
end
function c100280010.con(e,tp,eg,ep,ev,re,r,rp)
...
function c100280010.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if Duel.GetTurnPlayer()==tp then
		...
		if chk==0 then return Duel.IsExistingTarget(nil,tp,0,LOCATION_ONFIELD,1,nil) and Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
...
function c100280010.op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetTurnPlayer()==tp then
