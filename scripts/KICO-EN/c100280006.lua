...
function c100280006.initial_effect(c)
	...
	e1:SetCondition(c100280006.con)
	e1:SetTarget(c100280006.tg)
	e1:SetOperation(c100280006.op)
	c:RegisterEffect(e1)
end
function c100280006.con(e,tp,eg,ep,ev,re,r,rp)
...
function c100280006.tg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c100280006.op(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c100280006.checkfilter,tp,LOCATION_MZONE,0,nil)
	local check1=g:GetClassCount(Card.GetCode)==3
	if check1 and Duel.IsExistingMatchingCard(c100280006.thfilter,tp,LOCATION_DECK,0,1,nil) and (not Duel.IsExistingMatchingCard(c100280006.sumfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) or Duel.SelectYesNo(tp,aux.Stringid(100280006,1))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,c100280006.thfilter,tp,LOCATION_DECK,0,1,1,nil)
		if g:GetCount()>0 then
			Duel.SendtoHand(g,nil,REASON_EFFECT)
			Duel.ConfirmCards(1-tp,g)
			if Duel.IsExistingMatchingCard(c100280006.sumfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(100280006,2)) then
				Duel.BreakEffect()
				Duel.ShuffleHand(tp)
