...
function c101008066.cfilter(c,e,tp)
	...
	return bit.band(c:GetOriginalType(),TYPE_MONSTER)~=0 and c:IsReleasable()
		...
		and Duel.IsExistingMatchingCard(c101008066.spfilter,tp,0,LOCATION_GRAVE,1,nil,race,attr,e,tp)
end
function c101008066.spfilter(c,race,attr,e,tp)
...
function c101008066.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1-tp)
		and c101008066.spfilter(chkc,rc:GetOriginalRace(),rc:GetOriginalAttribute(),e,tp) end
	if chk==0 then
	...
	local sg=Duel.SelectTarget(tp,c101008066.spfilter,tp,0,LOCATION_GRAVE,1,1,nil,rc:GetOriginalRace(),rc:GetOriginalAttribute(),e,tp)
