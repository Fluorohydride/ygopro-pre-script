...
--
--By Tanaka Kotoha
function c101008065.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c101008065.target)
	e1:SetOperation(c101008065.operation)
	c:RegisterEffect(e1)
	--Equip limit
	...
	e2:SetValue(c101008065.eqlimit)
	c:RegisterEffect(e2)
	--immune
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_EQUIP)
	e3:SetCode(EFFECT_IMMUNE_EFFECT)
	e3:SetValue(c101008065.efilter)
	c:RegisterEffect(e3)
	--atk
	...
	e4:SetOperation(c101008065.atkop)
	c:RegisterEffect(e4)
end
function c101008065.eqlimit(e,c)
	return c:IsControler(e:GetHandlerPlayer()) and c:IsSummonType(SUMMON_TYPE_NORMAL)
end
function c101008065.filter(c)
	return c:IsFaceup() and c:IsSummonType(SUMMON_TYPE_NORMAL)
end
function c101008065.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c101008065.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101008065.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c101008065.filter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c101008065.operation(e,tp,eg,ep,ev,re,r,rp)
