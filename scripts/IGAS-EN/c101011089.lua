...
--Scripted by mallu11
function c101011089.initial_effect(c)
	--link summon
	aux.AddLinkProcedure(c,nil,2,2,c101011089.lcheck)
	c:EnableReviveLimit()
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101011089,0))
	...
	e1:SetCountLimit(1,101011089)
	e1:SetTarget(c101011089.sptg)
	e1:SetOperation(c101011089.spop)
	c:RegisterEffect(e1)
end
...
function c101011089.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c101011089.tgfilter,tp,LOCATION_MZONE,0,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
end
function c101011089.cfilter2(c)
	return c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)
end
function c101011089.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tgc=Duel.GetFirstTarget()
	local g=Duel.GetMatchingGroup(c101011089.cfilter2,tp,0,LOCATION_MZONE+LOCATION_GRAVE,nil)
	local tc=g:GetFirst()
	local attr=0
	while tc do
