...
function c101011000.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(101011000,0))
	e1:SetCategory(CATEGORY_ATKCHANGE+CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,101011000)
	e1:SetTarget(c101011000.destg)
	e1:SetOperation(c101011000.desop)
	c:RegisterEffect(e1)
end
function c101011000.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101011000.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tg:GetCount()>0 and Duel.Destroy(tg,REASON_EFFECT)~=0 and c:IsFaceup() and c:IsRelateToEffect(e) then
		...
		local ag=og:Filter(c101011000.atkfilter,nil)
