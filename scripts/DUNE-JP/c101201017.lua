...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SpecialSummon(g,SUMMON_VALUE_EVOLTILE,tp,tp,false,false,POS_FACEDOWN_DEFENSE)>0 and g:GetFirst():IsFacedown() then
...
function s.rfilter(c)
	return c:IsReleasableByEffect() and c:IsRace(RACE_REPTILE|RACE_DINOSAUR) and not c:IsStatus(STATUS_DESTROY_CONFIRMED|STATUS_BATTLE_DESTROYED)
end
function s.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return eg:IsExists(s.desrepfilter,1,nil,tp) and Duel.CheckReleaseGroup(tp,s.rfilter,1,nil)
	end
