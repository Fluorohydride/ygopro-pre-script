...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND,0,1,exc,e,tp)
	end
...
function s.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingTarget(s.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp,id)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
