...
function c101003055.initial_effect(c)
	...
	e1:SetCountLimit(1,101003055)
	e1:SetTarget(c101003055.target)
	e1:SetOperation(c101003055.activate)
	c:RegisterEffect(e1)
end
function c101003055.filter(c,tp)
...
function c101003055.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c101003055.filter,tp,LOCATION_MZONE,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c101003055.filter,tp,LOCATION_MZONE,0,1,1,nil,tp)
	e:SetLabelObject(g:GetFirst())
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,Card.IsFaceup,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,g)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c101003055.activate(e,tp,eg,ep,ev,re,r,rp)
