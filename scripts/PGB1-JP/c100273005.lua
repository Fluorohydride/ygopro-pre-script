...
function c100273005.initial_effect(c)
	...
	e2x:SetCondition(c100273005.eqcon)
...
function c100273005.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c100273005.filter,tp,0,LOCATION_MZONE,1,nil) end
...
function c100273005.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc1:IsRelateToEffect(e) and tc1:IsAbleToChangeControler() then
		...
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
		local sg=Duel.SelectMatchingCard(tp,c100273005.eqfilter,tp,LOCATION_MZONE,0,1,1,nil)
		if sg:GetCount()>0 then
			...
			if tc1:IsFaceup() and tc1:IsRelateToEffect(e) and tc1:IsControler(1-tp) and tc2 then
				...
				e2:SetValue(c100273005.eqlimit)
...
function c100273005.eqlimit(e,c)
