...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local hc,tc=Duel.GetFirstTarget()
	if hc~=e:GetLabelObject() then hc,tc=tc,hc end
	local chk=hc:IsType(TYPE_RITUAL)
	if hc:IsControler(tp) and hc:IsRelateToEffect(e)
		and Duel.SendtoHand(hc,nil,REASON_EFFECT)>0 and hc:IsLocation(LOCATION_HAND) and tc:IsControler(1-tp)
		and tc:IsRelateToEffect(e) and Duel.GetControl(tc,tp) and not chk then
