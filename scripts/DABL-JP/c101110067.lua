...
function c101110067.tktg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetCustomActivityCount(101110067,tp,ACTIVITY_SPSUMMON)==0
		and (b1 or b2)
...
function c101110067.tkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if b1 or b2 then
		...
		if Duel.SpecialSummon(token,0,to,to,false,false,POS_FACEUP)>0 then
...
function c101110067.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if b1 or b2 then
		...
		if sel==1 then to=1-tp end
		local token=Duel.CreateToken(tp,3285552)
		Duel.SpecialSummon(token,0,to,to,false,false,POS_FACEUP)
	end
end