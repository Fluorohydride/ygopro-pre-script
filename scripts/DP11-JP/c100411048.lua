	...
	if chk==0 then
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c100411048.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg3,mf,chkf)
			end
		end
		return res
	end
...
function c100411048.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg3=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c100411048.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
	end
