...
function c101105089.initial_effect(c)
	...
	e4:SetCost(c101105089.effcost)
	e4:SetTarget(c101105089.efftg)
	e4:SetOperation(c101105089.effop)
	c:RegisterEffect(e4)
end
--cannot target & indes
...
--choose effect
function c101105089.costfilter(c)
	return c:IsRace(RACE_INSECT)
end
function c101105089.effcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c101105089.costfilter,1,nil) end
	local g=Duel.SelectReleaseGroup(tp,c101105089.costfilter,1,1,nil)
...
function c101105089.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=e:GetHandler():GetFlagEffect(101105089)==0 end
	if chk==0 then return b1 or b2 end
	local off=1
	local ops,opval={},{}
	if b1 then
		ops[off]=aux.Stringid(101105089,0)
		opval[off]=0
		off=off+1
	end
	if b2 then
		ops[off]=aux.Stringid(101105089,1)
		opval[off]=1
		off=off+1
	end
...
function c101105089.effop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if sel==0 then
		...
		if c:IsFaceup() and c:IsRelateToEffect(e) then
			c:RegisterFlagEffect(101105089,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,0)
			local e1=Effect.CreateEffect(c)
			e1:SetType(EFFECT_TYPE_SINGLE)
