...
function c101010050.initial_effect(c)
    ...
    e2:SetCondition(c101010050.ctcon1)
    ...
    e2:SetCountLimit(1,101010050)
    e2:SetCost(c101010050.spcost)
    e2:SetTarget(c101010050.sptg)
    e2:SetOperation(c101010050.spop)
    c:RegisterEffect(e2)
end
...
function c101010050.ctcon1(e,tp,eg,ep,ev,re,r,rp)
    return e:GetHandler():IsSummonType(SUMMON_TYPE_LINK)
end
function c101010050.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return true end
    local ct=e:GetLabelObject():GetLabel()
    Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,ct,0,0x475)
end
function c101010050.ctop(e,tp,eg,ep,ev,re,r,rp)
    local ct=e:GetLabelObject():GetLabel()
    if e:GetHandler():IsRelateToEffect(e) then
        e:GetHandler():AddCounter(0x475,ct)
    end
end
function c101010050.atkval(e,c)
    return c:GetCounter(0x475)*200
end
function c101010050.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return e:GetHandler():IsCanRemoveCounter(tp,0x475,1,REASON_COST) end
    e:GetHandler():RemoveCounter(tp,0x475,1,REASON_COST)
end
