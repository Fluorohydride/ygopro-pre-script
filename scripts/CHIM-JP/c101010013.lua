...
function c101010013.initial_effect(c)
	...
	e1:SetCondition(c101010013.hspcon)
	e1:SetTarget(c101010013.hsptg)
	e1:SetOperation(c101010013.hspop)
	c:RegisterEffect(e1)
	--attack up
	...
	e3:SetDescription(aux.Stringid(101010013,1))
	e3:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_PHASE+PHASE_BATTLE)
	e3:SetRange(LOCATION_MZONE)
	e3:SetCondition(c101010013.spcon)
	e3:SetCost(c101010013.spcost)
	e3:SetTarget(c101010013.sptg)
	e3:SetOperation(c101010013.spop)
	c:RegisterEffect(e3)
end
function c101010013.hspcon(e,tp,eg,ep,ev,re,r,rp)
...
function c101010013.hspop(e,tp,eg,ep,ev,re,r,rp)
...
function c101010013.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToDeckAsCost() end
	Duel.SendtoDeck(c,nil,2,REASON_COST)
end
function c101010013.filter(c,e,tp)
...
function c101010013.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c101010013.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
	if tc then
