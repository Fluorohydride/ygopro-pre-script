...
function c101010018.initial_effect(c)
	...
	e2:SetCondition(c101010018.bdcon)
	...
	e3:SetCountLimit(1,101010118)
	e3:SetCondition(c101010018.rmcon)
	e3:SetTarget(c101010018.rmtg)
	e3:SetOperation(c101010018.rmop)
	c:RegisterEffect(e3)
end
function c101010018.cfilter(c,tp)
...
function c101010018.bdcon(e)
	local tp=e:GetHandlerPlayer()
	return Duel.GetLP(tp)>Duel.GetLP(1-tp)
end
function c101010018.rmcon(e,tp,eg,ep,ev,re,r,rp)
	return ep==tp
end
function c101010018.ctfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xc9)
end
function c101010018.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(1-tp) and chkc:IsAbleToRemove() end
	if chk==0 then return true end
	local ct=Duel.GetMatchingGroupCount(c101010018.ctfilter,tp,LOCATION_MZONE,0,nil)
	if ct>0 then
...
function c101010018.rmop(e,tp,eg,ep,ev,re,r,rp)
