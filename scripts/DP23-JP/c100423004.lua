...
function c100423004.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local res1=Duel.IsExistingMatchingCard(c100423004.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
	if not res1 then
		...
		if ce~=nil then
			...
			res1=Duel.IsExistingMatchingCard(c100423004.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
		end
	end
	local mg3=Duel.GetRitualMaterial(tp)
	aux.RCheckAdditional=c100423004.rcheck
...
function c100423004.activate(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
		...
		if ce~=nil then
			...
			mg2=fgroup(ce,e,tp)
			local mf=ce:GetValue()
			sg2=Duel.GetMatchingGroup(c100423004.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
		end
