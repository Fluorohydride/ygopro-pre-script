...
function c100336005.initial_effect(c)
	...
	e1:SetCondition(c100336005.spcon)
	c:RegisterEffect(e1)
	--search
...
function c100336005.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x102)
end
function c100336005.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c100336005.cfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c100336005.filter(c)
	return c:IsSetCard(0x102) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand()
end
function c100336005.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100336005.filter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c100336005.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c100336005.filter,tp,LOCATION_DECK,0,1,1,nil)
