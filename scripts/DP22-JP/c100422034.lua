...
function c100422034.filter2(c)
    return c:IsFaceup() and not c:IsLevel(8) and c:GetLevel()>0
end
function c100422034.lvtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    if chk==0 then return Duel.IsExistingMatchingCard(c100422034.filter2,tp,LOCATION_MZONE,0,1,nil) end
end
function c100422034.lvop(e,tp,eg,ep,ev,re,r,rp)
    local g=Duel.GetMatchingGroup(c100422034.filter1,tp,LOCATION_MZONE,0,nil)
    local lc=g:GetFirst()
    while lc do
        local e1=Effect.CreateEffect(e:GetHandler())
