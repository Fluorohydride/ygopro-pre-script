...
function c101102006.fselect(g,tg)
	return tg:IsExists(Card.IsLink,1,nil,#g)
end
function c101102006.spfilter(c,e,tp)
...
function c101102006.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		return cg:CheckSubGroup(c101102006.fselect,1,maxlink,tg)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local rg=cg:SelectSubGroup(tp,c101102006.fselect,false,1,maxlink,tg)
...
function c101102006.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c101102006.spfilter1,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,lk)
	local tc=g:GetFirst()
	if tc then
