...
function c101102007.fselect(g,e,tp)
	return Duel.IsExistingMatchingCard(c101102007.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,g:GetCount())
end
function c101102007.spfilter(c,e,tp,lk)
...
function c101102007.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetValue(c101102007.sumlimit)
	e2:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e2,tp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	local lk=e:GetLabel()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c101102007.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,lk)
	local tc=g:GetFirst()
	if tc then
