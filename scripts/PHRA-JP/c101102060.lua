...
--Scripted by mallu11
function c101102060.initial_effect(c)
	--activate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101102060,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCountLimit(1,101102060+EFFECT_COUNT_CODE_OATH)
	e1:SetCost(c101102060.cost)
	e1:SetTarget(c101102060.target)
	e1:SetOperation(c101102060.activate)
	c:RegisterEffect(e1)
	--to hand
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(101102060,1))
	...
	e2:SetTarget(c101102060.thtg)
	e2:SetOperation(c101102060.thop)
	c:RegisterEffect(e2)
end
function c101102060.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(100,0,0)
	return true
end
...
function c101102060.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101102060.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local check,code,lv=e:GetLabel()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(c101102060.spfilter),tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp,code,lv)
	local tc=g:GetFirst()
...
function c101102060.thop(e,tp,eg,ep,ev,re,r,rp)
