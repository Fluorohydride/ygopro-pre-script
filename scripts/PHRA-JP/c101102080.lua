...
--Scripted by mallu11
function c101102080.initial_effect(c)
	--Activate
	...
	e1:SetCountLimit(1,101102080+EFFECT_COUNT_CODE_OATH)
	e1:SetCondition(c101102080.condition)
	e1:SetTarget(c101102080.target)
	e1:SetOperation(c101102080.activate)
	c:RegisterEffect(e1)
end
function c101102080.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetLP(tp)<Duel.GetLP(1-tp)
end
function c101102080.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101102080.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetFieldGroup(tp,LOCATION_DECK,0)
	...
	if tc:IsType(TYPE_MONSTER) and (tc:IsLevel(1) or tc:IsLevel(8)) then
		...
		off=1
		if b1 then
			ops[off]=aux.Stringid(101102080,0)
			opval[off-1]=1
			off=off+1
		end
		if b2 then
			ops[off]=aux.Stringid(101102080,1)
			opval[off-1]=2
			off=off+1
		end
		if b3 then
			ops[off]=aux.Stringid(101102080,2)
			opval[off-1]=3
			off=off+1
		end
