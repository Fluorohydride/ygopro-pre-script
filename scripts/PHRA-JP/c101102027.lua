...
--Scripted by mallu11
function c101102027.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101102027,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,101102027)
	e1:SetTarget(c101102027.sptg)
	e1:SetOperation(c101102027.spop)
	c:RegisterEffect(e1)
	--chk
	...
	e0:SetCondition(c101102027.atkcon)
	e0:SetOperation(c101102027.chkop)
	c:RegisterEffect(e0)
	--atk/def
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(101102027,1))
	e2:SetCategory(CATEGORY_ATKCHANGE+CATEGORY_DEFCHANGE)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_BE_MATERIAL)
	e2:SetProperty(EFFECT_FLAG_DELAY)
	e2:SetCondition(c101102027.atkcon)
	e2:SetTarget(c101102027.atktg)
	e2:SetOperation(c101102027.atkop)
	c:RegisterEffect(e2)
