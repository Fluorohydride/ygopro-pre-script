...
function c100426018.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100426018,1))
	...
	e2:SetDescription(aux.Stringid(100426018,0))
...
function c100426018.sptg(e,tp,eg,ep,ev,re,r,rp,chkchkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingTarget(c100426018.thfilter,tp,LOCATION_MZONE,0,1,nil) and Duel.IsExistingTarget(aux.TRUE,tp,0,LOCATION_MZONE,1,nil) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
...
function c100426018.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		if hc:IsRelateToEffect(e) and hc:IsControler(tp) and Duel.SendtoHand(hc,nil,REASON_EFFECT)~=0
