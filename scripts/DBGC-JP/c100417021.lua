...
function c100417021.initial_effect(c)
	...
	e:SetCondition(c100417021.condition)
	e:SetCost(c100417021.cost)
	e:SetTarget(c100417021.target)
	e:SetOperation(c100417021.operation)
	c:RegisterEffect(e)
end
...
function c100417021.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if res and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and tc:IsType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(c100417021.spfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,tc:GetCode()) and tc:IsCanBeSpecialSummoned(e,0,tp,false,false) and Duel.SelectYesNo(tp,aux.Stringid(100417021,0)) then
		Duel.BreakEffect()
