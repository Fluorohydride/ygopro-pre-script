...
function c100909072.initial_effect(c)
	...
	e1:SetTarget(c100909072.target1)
	e1:SetOperation(c100909072.activate)
	c:RegisterEffect(e1)
	--special summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(100909072,0))
	...
	e2:SetCountLimit(1)
	e2:SetCost(c100909072.cost)
	e2:SetTarget(c100909072.target2)
	e2:SetOperation(c100909072.activate)
	c:RegisterEffect(e2)
	--search
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(100909072,1))
	e3:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCode(EVENT_TO_GRAVE)
	e3:SetCondition(c100909072.thcon)
	e3:SetTarget(c100909072.thtg)
	e3:SetOperation(c100909072.thop)
	c:RegisterEffect(e3)
end
function c100909072.cfilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_FUSION) and bit.band(c:GetSummonType(),SUMMON_TYPE_FUSION)==SUMMON_TYPE_FUSION
		and Duel.IsExistingTarget(c100909072.filter,tp,LOCATION_GRAVE,0,1,nil,c:GetLevel(),e,tp)
end
function c100909072.filter(c,lv,e,tp)
...
function c100909072.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetFlagEffect(100909072)==0 end
	e:GetHandler():RegisterFlagEffect(100909072,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
end
...
function c100909072.activate(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(100909072)==0 or not e:GetHandler():IsRelateToEffect(e) then return end
	local tc=Duel.GetFirstTarget()
	if tc:IsRelateToEffect(e) then
		Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c100909072.thcon(e,tp,eg,ep,ev,re,r,rp)
...
function c100909072.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100909072.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c100909072.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c100909072.thfilter,tp,LOCATION_DECK,0,1,1,nil)
