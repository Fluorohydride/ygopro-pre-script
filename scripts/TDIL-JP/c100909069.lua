...
function c100909069.filter1(c,tp,chk)
	return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsSetCard(0xad) and ((not chk)
		or Duel.IsExistingTarget(c100909069.filter2,tp,0,LOCATION_MZONE,1,nil))
end
function c100909069.filter2(c,tp,ex2)
	return c:IsFaceup() and c:IsType(TYPE_FUSION)
		and Duel.IsExistingMatchingCard(c100909069.filter3,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,ex2)
end
...
function c100909069.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c100909069.filter1,tp,LOCATION_MZONE,0,1,nil,tp,true) end
	local g1=Duel.SelectTarget(tp,c100909069.filter1,tp,LOCATION_MZONE,0,1,1,nil,tp,false)
	local g2=Duel.SelectTarget(tp,c100909069.filter2,tp,0,LOCATION_MZONE,1,1,nil,tp,g1:GetFirst())
