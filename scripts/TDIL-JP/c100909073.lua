...
function c100909073.initial_effect(c)
	...
	e2:SetTarget(c100909073.negtg)
	e2:SetOperation(c100909073.negop)
	c:RegisterEffect(e2)
end
c100909073.dark_magician_list=true
...
function c100909073.negcon(e,tp,eg,ep,ev,re,r,rp)
	return aux.exccon(e) and Duel.IsExistingMatchingCard(c100909073.cfilter,tp,LOCATION_MZONE,0,1,nil)
end
function c100909073.negcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost() end
	Duel.Remove(e:GetHandler(),POS_FACEUP,REASON_COST)
end
function c100909073.negfilter(c)
	return aux.disfilter1(c) and c:IsType(TYPE_SPELL+TYPE_TRAP)
end
function c100909073.negtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and c100909073.negfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c100909073.negfilter,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c100909073.negfilter,tp,0,LOCATION_ONFIELD,1,1,nil)
end
function c100909073.negop(e,tp,eg,ep,ev,re,r,rp)
