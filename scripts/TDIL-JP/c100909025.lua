...
function c100909025.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DESTROY+CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND)
	e1:SetCountLimit(1,100909025)
	e1:SetTarget(c100909025.sptg)
	e1:SetOperation(c100909025.spop)
	c:RegisterEffect(e1)
	--tohand
	...
	e2:SetCountLimit(1,100909925)
	e2:SetCondition(c100909025.thcon)
	e2:SetTarget(c100909025.thtg)
	e2:SetOperation(c100909025.thop)
	c:RegisterEffect(e2)
end
...
function c100909025.rmfilter(c,tp)
...
function c100909025.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,c100909025.desfilter2,tp,LOCATION_MZONE+LOCATION_HAND,0,1,1,nil,c)
	if g1:GetCount()<=0 then return end
	local c1=g1:GetFirst()
	local g2
	if c1:IsLocation(LOCATION_HAND) and ft<1 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
		g2=Duel.SelectMatchingCard(tp,c100909025.desfilter,tp,LOCATION_MZONE,0,1,1,c1,c)
	else
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
		g2=Duel.SelectMatchingCard(tp,c100909025.desfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,1,1,c1,c)
	end
	...
	if Duel.Destroy(g1,REASON_EFFECT)==2 then
		...
		if g1:FilterCount(Card.IsAttribute,nil,ATTRIBUTE_FIRE)==2 and g:GetCount()>0
			and Duel.SelectYesNo(tp,aux.Stringid(100909025,0)) then
			Duel.BreakEffect()
			local g=Duel.SelectMatchingCard(tp,c100909025.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,1,1,nil,tp)
			Duel.HintSelection(g)
			Duel.Remove(g,POS_FACEUP,REASON_EFFECT)
		end
	end
end
function c100909025.thcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_EFFECT)
end
function c100909025.thfilter(c)
	return not c:IsAttribute(ATTRIBUTE_FIRE) and c:IsRace(RACE_WYRM) and c:IsAbleToHand()
end
function c100909025.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(c100909025.thfilter,tp,LOCATION_GRAVE,0,nil)
	if chk==0 then return g:GetCount()>0 end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,g,1,0,0)
end
function c100909025.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c100909025.thfilter,tp,LOCATION_GRAVE,0,1,1,nil)
