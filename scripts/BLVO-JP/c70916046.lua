...
function c70916046.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c70916046.filter(chkc) end
	if chk==0 then return Duel.IsExistingMatchingCard(c70916046.filter,tp,0,LOCATION_MZONE,1,nil) end
	local t={}
	local i=1
...
function c70916046.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=Duel.GetMatchingGroup(c70916046.filter,tp,0,LOCATION_ONFIELD,nil)
