...
function c100427001.opa(e,tp,eg,ep,ev,re,r,rp)
	...
	local rec=dc*100
	e:SetLabel(dc)
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(c100427001.spfilter),tp,LOCATION_GRAVE,0,nil,e,tp,e:GetLabel())
	if Duel.Recover(tp,rec,REASON_EFFECT)>0 and g:GetCount()>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(c100427001,1)) then
		Duel.BreakEffect()
...
function c100427001.opd(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(c100427001,3)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
		local sg=g:Select(tp,1,1,nil)
		Duel.SendtoGrave(sg,REASON_EFFECT)
		local ct=ct-1
	end
	local op=Duel.SelectOption(tp,aux.Stringid(c100427001,4),aux.Stringid(c100427001,5))
