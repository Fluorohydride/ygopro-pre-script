...
function c700908015.initial_effect(c)
	...
	e1:SetCountLimit(1,700908015)
	e1:SetTarget(c700908015.sptg)
	e1:SetOperation(c700908015.spop)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
	e2:SetCode(EFFECT_CANNOT_BE_XYZ_MATERIAL)
	e2:SetValue(c700908015.xyzlimit)
	c:RegisterEffect(e2)
end
...
function c700908015.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c700908015.cfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c700908015.cfilter,tp,LOCATION_MZONE,0,1,1,nil)
	local c=g:GetFirst()
...
function c700908015.spop(e,tp,eg,ep,ev,re,r,rp)
