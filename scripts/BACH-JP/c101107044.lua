...
function c101107044.initial_effect(c)
	...
	e3:SetDescription(aux.Stringid(101107044,2))
...
function c101107044.spcon(e,tp,eg,ep,ev,re,r,rp,chk)
	return Duel.GetFirstMatchingCard(nil,tp,LOCATION_PZONE,0,e:GetHandler())
end
function c101107044.sptgfilter(c)
	return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and not c:IsForbidden()
end
...
function c101107044.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not e:GetHandler():IsRelateToEffect(e) or not tc or Duel.GetMZoneCount(tp)<1 then return end
...
function c101107044.ovltg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return (c:CheckRemoveOverlayCard(tp,2,REASON_EFFECT)
		...
		and rc:IsRelateToEffect(re) and re:GetHandler():IsCanBeXyzMaterial(c)
		and c:GetFlagEffect(101107044)==0 end
	c:RegisterFlagEffect(101107044,RESET_CHAIN,0,1)
end
function c101107044.ovlop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local rc=re:GetHandler()
	local opt1=c:CheckRemoveOverlayCard(tp,2,REASON_EFFECT)
	...
	if opt1 and opt2 then result=Duel.SelectOption(tp,aux.Stringid(101107044,3),aux.Stringid(101107044,4)) end
	if result==0 then
	...
	if result>0 and rc:IsRelateToEffect(re) then
		Duel.Overlay(c,rc)
	end
end
