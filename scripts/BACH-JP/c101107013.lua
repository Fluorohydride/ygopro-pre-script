...
function c101107013.initial_effect(c)
	...
	e3:SetCountLimit(1,101107013+100)
	...
	e4:SetCountLimit(1,101107013+200)
...
function c101107013.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101107013.thfilter,tp,LOCATION_DECK,0,nil) end
...
function c101107013.filter1(c)
	return c:IsFaceup() and c:IsRace(RACE_FIEND)
end
function c101107013.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c101107013.filter1(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101107013.filter1,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c101107013.filter1,tp,LOCATION_MZONE,0,1,1,nil)
end
function c101107013.filter2(c)
	return (c:IsFaceup() or c:IsLocation(LOCATION_GRAVE)) and c:IsSetCard(0xae)
end
function c101107013.atkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	local ct=Duel.GetMatchingGroupCount(c101107013.filter2,tp,LOCATION_MZONE+LOCATION_GRAVE,0,nil)
