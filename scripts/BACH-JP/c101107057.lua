...
function c101107057.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(c101107057.spfilter),tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp) end
...
function c101107057.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:GetCount()>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP) 
		and Duel.IsExistingMatchingCard(c101107057.cfilter,tp,LOCATION_MZONE,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(101107057,0)) then
		Duel.BreakEffect()
...
function c101107057.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c101107057.xyzfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(c101107057.mfilter,tp,LOCATION_GRAVE,0,1,nil) end
...
function c101107057.matop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
		local g=Duel.SelectMatchingCard(tp,c101107057.mfilter,tp,LOCATION_GRAVE,0,1,1,nil)
