...
function c100200172.atkcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_ATKCHANGE,e:GetHandler(),1,0,LOCATION_MZONE)
    	Duel.SetOperationInfo(0,CATEGORY_DEFCHANGE,e:GetHandler(),1,0,LOCATION_MZONE)
end
...
function c100200172.filter(c)
	return c:IsFaceup() and not (c:IsHasEffect(EFFECT_EXTRA_ATTACK) or c:IsHasEffect(EFFECT_EXTRA_ATTACK_MONSTER))
end
