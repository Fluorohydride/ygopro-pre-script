...
function c101006046.initial_effect(c)
    c:EnableReviveLimit()
    aux.AddLinkProcedure(c,aux.FilterBoolFunctionEx(Card.IsType,TYPE_EFFECT),2,nil,c101006046.matcheck)
    --atk up
...
function c101006046.seqfilter(c)
    return c:IsFaceup() and c:GetSequence()<5 and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
end
...
function c101006046.seqop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if not tc:IsRelateToEffect(e) or tc:IsImmuneToEffect(e) or Duel.GetLocationCount(tc:GetControler(),LOCATION_MZONE)<=0 then return end
    local p1,p2
