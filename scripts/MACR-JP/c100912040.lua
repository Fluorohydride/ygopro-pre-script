...
function c100912040.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcFun2(c,aux.FilterBoolFunction(Card.IsFusionSetCard,0x99),aux.FilterBoolFunction(c100912040.ffilter),true)
	--destroy
	...
	if ct>0 and Duel.Damage(p,ct*200,REASON_EFFECT)~=0 and e:GetLabel()==1 then
		...
		if g:GetCount()>0 then
			...
			if Duel.Destroy(g,REASON_EFFECT)~=0 then
				local dam=g:GetFiest():GetBaseAttack()
				Duel.Damage(dam,d,REASON_EFFECT)
			end
		end
	end
end
