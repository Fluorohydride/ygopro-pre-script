...
--Scripted by mallu11
function c101105079.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_NEGATE+CATEGORY_TODECK)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_CHAINING)
	e1:SetCondition(c101105079.condition)
	e1:SetCost(c101105079.cost)
	e1:SetTarget(c101105079.target)
	e1:SetOperation(c101105079.activate)
	c:RegisterEffect(e1)
end
function c101105079.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c101105079.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetDecktopGroup(tp,3)
	if chk==0 then return g:FilterCount(Card.IsAbleToRemoveAsCost,nil,POS_FACEDOWN)==3 end
	Duel.DisableShuffleCheck()
	Duel.Remove(g,POS_FACEDOWN,REASON_COST)
end
function c101105079.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101105079.activate(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
