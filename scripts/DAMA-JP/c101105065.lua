...
function c101105065.initial_effect(c)
	...
	e1:SetCountLimit(1,101105065+EFFECT_COUNT_CODE_OATH)
...
function c101105065.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg=Duel.GetRitualMaterial(tp):Filter(c101105065.matfilter,nil)
...
function c101105065.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if Duel.SpecialSummonStep(tc,SUMMON_TYPE_RITUAL,tp,tp,false,true,POS_FACEUP) then
			...
			tc:RegisterFlagEffect(101105065,RESET_EVENT+RESETS_STANDARD,0,1,fid)
...
function c101105065.tdcon(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffectLabel(101105065)==e:GetLabel() then
		return true
	else
		e:Reset()
		return false
