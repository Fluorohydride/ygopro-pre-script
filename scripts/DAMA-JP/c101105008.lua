...
--Scripted by mallu11
function c101105008.initial_effect(c)
	--change name
	aux.EnableChangeCode(c,68468459,LOCATION_MZONE+LOCATION_GRAVE)
	--to grave
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101105008,0))
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_HAND+LOCATION_GRAVE)
	e1:SetCountLimit(1,101105008)
	e1:SetTarget(c101105008.target)
	e1:SetOperation(c101105008.operation)
	c:RegisterEffect(e1)
end
function c101105008.costfilter(c)
...
function c101105008.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g2=Duel.GetMatchingGroup(c101105008.costfilter,tp,LOCATION_DECK,0,nil)
	if b1 then
		g:Merge(g1)
	end
	if b2 then
...
function c101105008.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
