...
function c101105033.initial_effect(c)
	...
	e00:SetValue(c101105033.atkval)
	c:RegisterEffect(e00)
	--negate
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101105033,0))
	e1:SetCategory(CATEGORY_NEGATE+CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_CHAINING)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1,101105033)
	e1:SetCondition(c101105033.condition)
	e1:SetTarget(c101105033.target)
	e1:SetOperation(c101105033.activate)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetCondition(c101105033.matcon)
	e2:SetOperation(c101105033.matop)
	c:RegisterEffect(e2)
	e0:SetLabelObject(e2)
	--search
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(101105033,1))
	e3:SetCategory(CATEGORY_TOHAND+CATEGORY_SEARCH)
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_TO_GRAVE)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCountLimit(1,101105033)
...
function c101105033.attfilter(c,rc)
	return c:GetAttribute()>0
end
function c101105033.valcheck(e,c)
	local mg=c:GetMaterial()
	local fg=mg:Filter(c101105033.attfilter,nil,c)
...
function c101105033.condition(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and re:IsActiveType(TYPE_MONSTER) and Duel.IsChainNegatable(ev)
		and Duel.IsExistingMatchingCard(Card.IsAttribute,tp,LOCATION_GRAVE,0,1,nil,re:GetHandler():GetAttribute()) and e:GetHandler():GetFlagEffect(101105033)>0
end
