...
function c101105032.cfilter(c)
	return (c:IsType(TYPE_NORMAL) or c:IsSetCard(0x18f) and c:IsType(TYPE_MONSTER))
end
function c101105032.discon(e,tp,eg,ep,ev,re,r,rp)
	local c,att=e:GetHandler(),0
	...
	return tc and tc:IsFaceup() and tc:IsControler(1-tp) and tc:IsType(TYPE_MONSTER)
		and tc:IsAttribute(att) and not tc:IsStatus(STATUS_DISABLED)
end
...
function c101105032.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoDeck(g,nil,1,REASON_EFFECT)~=0 then
		if tc:IsRelateToBattle() and tc:IsFaceup() and tc:IsControler(1-tp) and not tc:IsStatus(STATUS_DISABLED) then
