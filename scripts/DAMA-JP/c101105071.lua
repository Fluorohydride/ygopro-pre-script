...
--Scripted by mallu11
function c101105071.initial_effect(c)
	--Activate
	...
	e1:SetDescription(aux.Stringid(101105071,0))
	e1:SetCategory(CATEGORY_TOEXTRA+CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_CHAINING)
	e1:SetRange(LOCATION_SZONE)
	e1:SetCountLimit(1,101105071)
	e1:SetCondition(c101105071.condition)
	e1:SetTarget(c101105071.target)
	e1:SetOperation(c101105071.activate)
	c:RegisterEffect(e1)
	--attack down
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(101105071,1))
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetProperty(EFFECT_FLAG_DELAY)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1,101105171)
	e2:SetCondition(c101105071.atkcon)
	e2:SetTarget(c101105071.atktg)
	e2:SetOperation(c101105071.atkop)
	c:RegisterEffect(e2)
end
function c101105071.condition(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp
end
function c101105071.spfilter(c,e,tp)
...
function c101105071.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SendtoDeck(tc,nil,2,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_EXTRA) then
		...
		if Duel.IsChainDisablable(ev) and (Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
			...
			local sg=Duel.SelectMatchingCard(tp,c101105071.spsfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp,true)
...
function c101105071.atktg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsFaceup,tp,0,LOCATION_MZONE,1,nil) end
end
function c101105071.atkop(e,tp,eg,ep,ev,re,r,rp)
