...
--Scripted by mallu11
function c101105040.initial_effect(c)
	c:SetSPSummonOnce(101105040)
	--synchro summon
	aux.AddSynchroProcedure(c,nil,aux.NonTuner(Card.IsSetCard,0x10),1)
	c:EnableReviveLimit()
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101105040,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SPSUMMON_SUCCESS)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	e1:SetCondition(c101105040.spcon)
	e1:SetTarget(c101105040.sptg)
	e1:SetOperation(c101105040.spop)
	c:RegisterEffect(e1)
	--cannot be target
	...
	e2:SetTarget(c101105040.imval)
	e2:SetValue(aux.tgoval)
	c:RegisterEffect(e2)
end
function c101105040.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO)
end
function c101105040.spfilter(c,e,tp)
	return c:IsSetCard(0x10) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c101105040.fselect(g,tp)
	return g:GetClassCount(Card.GetLocation)==#g and Duel.IsExistingMatchingCard(c101105040.synfilter,tp,LOCATION_EXTRA,0,1,nil,g)
end
function c101105040.synfilter(c,g)
	return c:IsSetCard(0x10) and c:IsSynchroSummonable(nil,g)
end
function c101105040.chkfilter(c,tp)
	return c:IsType(TYPE_SYNCHRO) and c:IsSetCard(0x10) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
end
function c101105040.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		if not Duel.IsPlayerCanSpecialSummonCount(tp,2) then return false end
		if Duel.IsPlayerAffectedByEffect(tp,59822133) then return false end
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=1 then return false end
		local cg=Duel.GetMatchingGroup(c101105040.chkfilter,tp,LOCATION_EXTRA,0,nil,tp)
...
function c101105040.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerCanSpecialSummonCount(tp,2) and not Duel.IsPlayerAffectedByEffect(tp,59822133) and Duel.GetLocationCount(tp,LOCATION_MZONE)>1 then
		...
		if #cg>0 then
			...
			local sg=g:SelectSubGroup(tp,c101105040.fselect,false,2,2,tp)
			if sg then
	...
	e3:SetTarget(c101105040.splimit)
	e3:SetReset(RESET_PHASE+PHASE_END)
	Duel.RegisterEffect(e3,tp)
end
function c101105040.splimit(e,c)
	return not c:IsAttribute(ATTRIBUTE_WIND)
end
function c101105040.imval(e,c)
