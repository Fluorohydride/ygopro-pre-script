...
function c101105072.ffilter1(c,e)
...
function c101105072.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local chkf=tp
	local fmg1=Duel.GetFusionMaterial(tp):Filter(c101105072.ffilter1,nil,e)
	local fsg1=Duel.GetMatchingGroup(c101105072.ffilter2,tp,LOCATION_EXTRA,0,nil,e,tp,fmg1,nil,chkf)
	local fmg2=nil
	local fsg2=nil
	...
	if ce~=nil then
		...
		fsg2=Duel.GetMatchingGroup(c101105072.ffilter2,tp,LOCATION_EXTRA,0,nil,e,tp,fmg2,mf,chkf)
	end
	local rmg1=Duel.GetRitualMaterial(tp)
	local rsg=Duel.GetMatchingGroup(aux.RitualUltimateFilter,tp,LOCATION_HAND,0,nil,c101105072.rfilter,e,tp,rmg1,nil,Card.GetLevel,"Greater")
	local off=1
	local ops={}
	local opval={}
	ops[off]=aux.Stringid(101105072,0)
	opval[off-1]=0
	off=off+1
	if fsg1:GetCount()>0 or (fsg2~=nil and fsg2:GetCount()>0) then
		ops[off]=aux.Stringid(101105072,1)
		opval[off-1]=1
		off=off+1
	end
	if rsg:GetCount()>0 then
		ops[off]=aux.Stringid(101105072,2)
		opval[off-1]=2
		off=off+1
	end
