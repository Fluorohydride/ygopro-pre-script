...
--Scripted by mallu11
function c101105009.initial_effect(c)
	--spsummon
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(101105009,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetRange(LOCATION_HAND)
	e1:SetHintTiming(0,TIMINGS_CHECK_MONSTER+TIMING_MAIN_END)
	e1:SetCountLimit(1,101105009)
	e1:SetCondition(c101105009.spcon)
	e1:SetTarget(c101105009.sptg)
	e1:SetOperation(c101105009.spop)
	c:RegisterEffect(e1)
	--destroy
	...
	e2:SetCountLimit(1,101105109)
	e2:SetCondition(c101105009.descon)
	e2:SetTarget(c101105009.destg)
	e2:SetOperation(c101105009.desop)
	c:RegisterEffect(e2)
end
...
function c101105009.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return (ph==PHASE_MAIN1 or ph==PHASE_MAIN2)
		and Duel.IsExistingMatchingCard(c101105009.cfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
end
function c101105009.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101105009.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c101105009.descon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(Card.IsSummonPlayer,1,nil,1-tp)
end
function c101105009.desfilter(c,tp)
...
function c101105009.desop(e,tp,eg,ep,ev,re,r,rp)
