...
function c100206010.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.CheckLocation(tp,LOCATION_MZONE,seq)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,100206010,0,TYPES_NORMAL_TRAP_MONSTER,2100,100,5,RACE_INSECT,ATTRIBUTE_EARTH) end
...
function c100206010.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,true,false,POS_FACEUP,zone)~=0 then
		local g=c:GetColumnGroup():Filter(c100206010.filter,nil,tp)
