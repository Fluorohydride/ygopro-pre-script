...
function c100225017.desfilter1(c,ft)
...
function c100225017.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and chkc~=c and c100225017.desfilter1(chkc,ft) end
	if chk==0 then
		return ft>-1 and Duel.IsExistingTarget(c100225017.desfilter1,tp,LOCATION_ONFIELD,0,1,c,ft)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c100225017.desfilter1,tp,LOCATION_ONFIELD,0,1,1,c,ft)
...
function c100225017.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)~=0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,c100225017.spfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp,tc:GetOriginalCode())
