...
function c101104063.initial_effect(c)
	...
	e1:SetCountLimit(1,101104063+EFFECT_COUNT_CODE_OATH)
...
function c101104063.filter1(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsDefense(200) and c:IsType(TYPE_TUNER) and c:IsAbleToRemove()
		and Duel.IsExistingTarget(c101104063.filter2,tp,LOCATION_GRAVE,0,1,c,e,tp,c:GetLevel())
end
function c101104063.filter2(c,e,tp,lv)
	return c:IsAttribute(ATTRIBUTE_FIRE) and c:IsDefense(200) and not c:IsType(TYPE_TUNER) and c:IsLevelAbove(0) and c:IsAbleToRemove()
		and Duel.IsExistingMatchingCard(c101104063.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,c:GetLevel()+lv)
end
function c101104063.spfilter(c,e,tp,lv)
...
function c101104063.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g2=Duel.SelectTarget(tp,c101104063.filter2,tp,LOCATION_GRAVE,0,1,1,g1:GetFirst(),e,tp,g1:GetFirst():GetLevel())
	g1:Merge(g2)
...
function c101104063.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(tg,POS_FACEUP,REASON_EFFECT)==2 then
		...
		local sg=Duel.SelectMatchingCard(tp,c101104063.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,lv)
		if #sg>0 then
