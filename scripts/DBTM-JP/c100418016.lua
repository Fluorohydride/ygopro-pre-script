...
function c100418016.spfilter(c,e,tp)
	return not c:IsCode(100418016) and c:IsRace(RACE_FIEND) and c:IsLevelBelow(4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
...
function c100418016.drcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c100418016.cfilter,1,nil,tp,re,r,rp) and not eg:IsContains(e:GetHandler())
end
...
function c100418016.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Draw(p,d,REASON_EFFECT)>0 then
		...
		if (#spg>0 or #stg>0) and Duel.SelectYesNo(tp,aux.Stringid(100418016,0)) then
			local op=0
			if #spg>0 and #stg==0 then
				op=Duel.SelectOption(tp,aux.Stringid(100418016,1))+1
			end
			if #spg==0 and #stg>0 then
				op=Duel.SelectOption(tp,aux.Stringid(100418016,2))+2
			end
			if #spg>0 and #stg>0 then
				op=Duel.SelectOption(tp,aux.Stringid(100418016,1),aux.Stringid(100418016,2))+1
			end
			if op==1 then
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
