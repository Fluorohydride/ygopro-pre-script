...
function c100425005.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(100425005,1))
...
function c100425005.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if  yc>0 and Duel.SelectYesNo(tp,aux.Stringid(100425005,2)) then
...
function c100425005.spfilter(c,e,tp,mc)
	return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsType(TYPE_SYNCHRO)
		and c:IsLevel(7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP)
...
function c100425005.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c100425005.spfilter,tp,LOCATION_EXTRA,0,1,2,nil,e,tp,nil)
