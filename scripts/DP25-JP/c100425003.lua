--HSRコルクー10
--Script by XyLeN
function c100425003.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,nil,aux.NonTuner(nil),1)
	c:EnableReviveLimit()
	--choose effect
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(100425003,0))
	...
	e2:SetValue(c100425003.valcheck)
	e2:SetLabelObject(e1)
	c:RegisterEffect(e2)
	Duel.AddCustomActivityCounter(100425003,ACTIVITY_SPSUMMON,c100425003.counterfilter)
end
function c100425003.counterfilter(c)
...
function c100425003.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local ops,opval={},{}
	if b1 then
		ops[off]=aux.Stringid(100425003,1)
		opval[off]=0
		off=off+1
	end
	if b2 then
		ops[off]=aux.Stringid(100425003,2)
		opval[off]=1
		off=off+1
	end
...
function c100425003.operation(e,tp,eg,ep,ev,re,r,rp)
	local sel=e:GetLabel()
	if sel==0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,c100425003.thfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c100425003.valcheck(e,c)
	local g=c:GetMaterial()
	if g:GetCount()>0 and not g:IsExists(c100425003.mfilter,1,nil) then
