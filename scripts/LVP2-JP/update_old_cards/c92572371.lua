...
function c92572371.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return sg:IsExists(c92572371.cfilter1,1,nil,sg,tp)
		or (Duel.IsPlayerAffectedByEffect(tp,100235056) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0) end
	if sg:IsExists(c92572371.cfilter1,1,nil,sg,tp)
		and (not Duel.IsPlayerAffectedByEffect(tp,100235056) or Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not Duel.SelectYesNo(tp,aux.Stringid(100235056,0))) then
