...
--Scripted by mallu11
function c101101075.initial_effect(c)
	--activate
	...
	e1:SetCountLimit(1,101101075+EFFECT_COUNT_CODE_OATH)
	e1:SetCost(c101101075.cost)
	e1:SetTarget(c101101075.target)
	e1:SetOperation(c101101075.activate)
	c:RegisterEffect(e1)
end
function c101101075.filter1(c,e)
	return not c:IsImmuneToEffect(e)
end
function c101101075.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsAttribute(ATTRIBUTE_DARK) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,chkf)
end
function c101101075.costfilter(c,e,tp)
	local chkf=tp
	local mg1=Duel.GetFusionMaterial(tp)
	if mg1:IsContains(c) then mg1:RemoveCard(c) end
	local res=Duel.IsExistingMatchingCard(c101101075.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
	if not res then
...
function c101101075.activate(e,tp,eg,ep,ev,re,r,rp)
	local chkf=tp
	local mg1=Duel.GetFusionMaterial(tp):Filter(c101101075.filter1,nil,e)
	local sg1=Duel.GetMatchingGroup(c101101075.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101101075.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		local label,rec=e:GetLabel()
		if e:IsHasType(EFFECT_TYPE_ACTIVATE) and label==100 and rec>0 and Duel.SelectYesNo(tp,aux.Stringid(101101075,0)) then
			Duel.BreakEffect()
