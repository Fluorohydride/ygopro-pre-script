...
--Scripted by mallu11
function c101101039.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	...
	e1:SetDescription(aux.Stringid(101101039,0))
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e1:SetCode(EVENT_SPSUMMON_SUCCESS)
	e1:SetProperty(EFFECT_FLAG_DELAY)
	e1:SetCondition(c101101039.descon)
	e1:SetTarget(c101101039.destg)
	e1:SetOperation(c101101039.desop)
	c:RegisterEffect(e1)
	--atk
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(101101039,1))
	...
	e2:SetOperation(c101101039.atkop)
	c:RegisterEffect(e2)
end
function c101101039.descon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_FUSION)
end
function c101101039.destg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101101039.desop(e,tp,eg,ep,ev,re,r,rp)
	local g1=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_ONFIELD,0,nil)
	local g2=Duel.GetMatchingGroup(aux.TRUE,tp,0,LOCATION_ONFIELD,nil)
...
function c101101039.atkfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:GetBaseAttack()>0
end
function c101101039.tgfilter(c)
	return c:IsSetCard(0xad) and not c:IsCode(101101039) and c:IsAbleToGrave()
end
...
function c101101039.atkop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,c101101039.tgfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,1,1,nil)
	local gc=g:GetFirst()
