...
function c101002053.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c101002053.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101002053.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c101002053.filter,tp,LOCATION_MZONE,0,1,1,nil)
end
